...
export async function getCachedRounds(date: string): Promise<RoundData[] | null> {
  const { data, error } = await supabaseServer
    ...
    .single();
  if (error) {
    console.error('getCachedRounds error:', error.message);
    return null;
  }
  return data ? (data.rounds as RoundData[]) : null;
}

export async function setCachedRounds(date: string, rounds: RoundData[]): Promise<void> {
  const { error } = await supabaseServer
    .from('rxsordle_rounds')
    .upsert({ date, rounds }, { onConflict: 'date' });
  if (error) console.error('setCachedRounds error:', error.message);
}
...
// ── Main generation function — called by both /api/generate and /api/admin/regenerate ──
export async function generateRoundsForDate(date: string, salt = 0): Promise<RoundData[]> {
  const baseSeed = dateToSeed(date) + salt;
